...
export const Login = () => {
  ...
  useEffect(() => {
    if (isLoggedIn && user) {
      if (user.role === "ADMIN") {
        ...
        navigate(
          redirect === "admin" || redirect === "seller/dashboard"
            ? "/dashboard"
            : `/${redirect}`,
        );
      }
  ...
  const handleSendOtp = async (e) => {
    ...
    try {
      ...
      addToast(
        err.message || "Failed to send verification code. Please retry.",
        "error",
      );
    }
  };
  ...
  const handleVerifyOtp = async (e) => {
    ...
    try {
      ...
      addToast(
        err.message || "Invalid verification code. Please verify and retry.",
        "error",
      );
    }
  };
      ...
      <div className="bg-white border border-gray-100 rounded-card p-8 shadow-card space-y-6">
        ...
        {!otpSent ? (
          <div>
            ...
            <form onSubmit={handleSendOtp} className="space-y-4">
              ...
              <p className="text-[10px] text-swift-mid leading-relaxed">
                By proceeding, you agree to SwiftCart’s Terms of Use and Privacy
                Policy. Standard carrier messaging charges may apply.
              </p>
            ...
            <div className="relative my-6 flex items-center justify-center">
              ...
              <span className="relative bg-white px-3 text-[10px] text-swift-mid uppercase font-bold tracking-wider">
                Or continue with
              </span>
            </div>
          ...
          <form onSubmit={handleVerifyOtp} className="space-y-5">
            <div>
              <label className="block text-xs font-bold text-swift-dark mb-1.5 text-center">
                Enter 6-digit OTP
              </label>
              <input
                type="text"
