  ...
  const handleInvoiceDownload = async (orderId) => {
    try {
      const response = await fetch(
        `${API_BASE_URL}/api/v1/orders/${orderId}/invoice`,
        {
          headers: {
      ...
      addToast(
        `Invoice PDF receipt for order ${orderId} downloaded successfully`,
        "success",
      );
    } catch (err) {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 md:px-6 py-8 pb-20">
      <div className="flex flex-col sm:flex-row items-center gap-4 border-b border-gray-150 pb-6 mb-8 text-center sm:text-left">
        ...
        <div
          className={`w-16 h-16 bg-swift-orange text-white rounded-full flex items-center justify-center font-bold text-2xl font-heading shadow-md avatar-fallback flex-shrink-0 ${user?.avatarUrl ? "hidden" : ""}`}
        >
          {user?.name
            ? user.name
                .split(" ")
        ...
        <div className="min-w-0">
          <div className="flex flex-col sm:flex-row items-center gap-2">
            <h2 className="font-heading font-extrabold text-xl text-swift-dark truncate max-w-full">
              {user?.name}
            </h2>
          ...
          <p className="text-xs text-swift-mid mt-1.5 leading-relaxed break-all">
            {user?.phone ? `Mobile: +91 ${user.phone} • ` : ""}Email:{" "}
            {user?.email}
          </p>
        </div>
      </div>
              ...
              {!selectedOrder ? (
                        ...
                        >
                          ...
                          <div className="bg-gray-50 px-4 py-3 flex flex-wrap items-center justify-between gap-3 text-xs text-swift-mid border-b border-gray-150 font-bold">
                            <div className="flex gap-4">
                              <div>
                                <span className="uppercase text-[10px] text-swift-mid font-medium block">
                                  Order Placed
                                </span>
                            ...
                            <div className="flex items-center gap-3">
                              ...
                              <div className="text-left">
                                ...
                                <div className="flex items-center gap-2 mt-1">
                                  <span
                                    className={`inline-block text-[9px] font-extrabold uppercase px-2 py-0.5 rounded-pill ${
                                      order.status === "Delivered"
                                        ? "bg-swift-green/10 text-swift-green border border-swift-green/20"
                                        : order.status === "Cancelled"
                                          ? "bg-swift-red/10 text-swift-red border border-swift-red/20"
                ...
                <div className="space-y-6 text-left">
                  ...
                  <div className="flex flex-wrap justify-between items-baseline gap-2 border-b border-gray-100 pb-3">
                    <div>
                      <h4 className="font-heading font-extrabold text-lg text-swift-dark">
                        Order Details
                      </h4>
                      <p className="text-xs text-swift-mid">
                        Placed on {selectedOrder.date} • Reference:{" "}
                        <span className="font-mono font-bold text-swift-dark">
                          {selectedOrder.id}
                        </span>
                      </p>
                    </div>
                  ...
                  <div className="bg-gray-50 border border-gray-150 rounded-card p-6">
                    <span className="block text-[10px] font-bold text-swift-mid uppercase tracking-wide mb-4">
                    ...
                    <div className="flex flex-col sm:flex-row justify-between items-start gap-4 sm:gap-2 relative">
                      {selectedOrder.trackingTimeline.map((step, idx) => {
                        return (
                          <div
                            key={idx}
                            className="flex sm:flex-col items-center gap-3 sm:gap-1.5 sm:text-center flex-grow"
                          >
                            <div
                            ...
                            <div>
                              ...
                              <div className="text-[10px] text-swift-mid max-w-[150px]">
                                {step.description}
                              </div>
                              {step.date !== "Pending" && (
                                <div className="text-[9px] text-swift-orange font-mono font-semibold">
                                  {step.date}
                                </div>
                              )}
                            </div>
                  ...
                  {/* Items detail list */}
                  <div className="space-y-4">
                    <span className="block text-[10px] font-bold text-swift-mid uppercase tracking-wide">
                    ...
                    {selectedOrder.items.map((item, idx) => (
                      ...
                      >
                        ...
                        <div className="flex-grow">
                          <h5 className="text-sm font-bold text-swift-dark">
                            {item.name}
                          </h5>
                          <p className="text-xs text-swift-mid">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 pt-4">
                    <div className="space-y-1.5 text-xs text-swift-mid">
                      ...
                      <div className="font-bold text-swift-dark">
                ...
                <div className="text-center py-20 text-swift-mid space-y-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-swift-mid mt-0.5">
                      Save items you like to track price cuts and stocks.
                    </p>
                  </div>
                  <Link
                    to="/"
                    className="inline-block px-6 py-2 bg-swift-orange text-white rounded-button text-xs font-bold"
                  >
                    Start Shopping
                  </Link>
                </div>
            ...
            <div className="space-y-6 text-left">
              ...
              {showAddrForm && (
                <form
                  ...
                  className="border border-gray-150 rounded-card p-4 bg-gray-50/50 space-y-4"
                >
                  <span className="block text-xs font-extrabold text-swift-dark uppercase tracking-wider">
                    New Delivery Coordinates
                  </span>
                ...
                {addresses.map((addr) => (
                  <div
                    ...
                    className={`border rounded-card p-4 relative flex flex-col justify-between gap-3 bg-white hover:border-gray-300 transition-colors ${
                      addr.isDefault
                        ? "border-swift-green ring-1 ring-swift-green/10"
                        : "border-gray-200"
                    }`}
                  >
                    <div>
