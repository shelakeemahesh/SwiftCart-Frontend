...
export const ProductDetail = () => {
      ...
      try {
        ...
        try {
          const related = await apiClient.get(
            `/api/v1/products/${prod.id}/related?limit=4`,
          );
  ...
  const handlePincodeSubmit = (e) => {
    ...
    if (/^\d{6}$/.test(pincodeCheck)) {
      ...
      setDeliveryResult(
        `Guaranteed delivery by ${date.toLocaleDateString("en-IN", options)} | Free Shipping`,
      );
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 md:px-6 py-6 pb-24">
      {/* Breadcrumb Header */}
      <nav
        className="text-xs font-bold text-swift-mid mb-6"
        aria-label="Breadcrumb"
      >
        <Link to="/" className="hover:text-swift-orange">
          Home
        </Link>
        <span className="mx-2">&gt;</span>
        <Link
          to={`/category/${product.category}`}
          className="hover:text-swift-orange capitalize"
        >
          {product.category}
        </Link>
        <span className="mx-2">&gt;</span>
        <span className="text-swift-dark truncate max-w-[200px] inline-block align-bottom">
          {product.name}
        </span>
      </nav>
          ...
          <div className="bg-white border border-gray-100 rounded-card p-4 shadow-xs relative">
            ...
            <button
              ...
              className={`absolute top-4 right-4 z-20 p-2.5 rounded-full border border-gray-100 shadow-sm bg-white hover:scale-105 transition-all ${
                isSaved
                  ? "text-swift-red"
                  : "text-swift-mid hover:text-swift-red"
              }`}
          ...
          <div className="flex gap-2.5 overflow-x-auto py-1 no-scrollbar">
            {product.images.map((img, idx) => (
              <button
                ...
                className={`w-16 h-16 sm:w-20 sm:h-20 border-2 rounded-button overflow-hidden bg-white hover:border-swift-orange flex-shrink-0 transition-all ${
                  activeMediaIdx === idx && !showVideo
                    ? "border-swift-orange scale-95 shadow-sm"
                    : "border-gray-100"
                }`}
              >
                <img
            ...
            {product.videoUrl && (
              ...
              >
                ...
                <span className="text-[9px] uppercase font-bold tracking-wider mt-1 text-gray-300">
                  Video
                </span>
              </button>
            )}
        ...
        <div className="lg:col-span-6 space-y-6">
          ...
          <div className="bg-white border border-gray-100 rounded-card p-4 shadow-xs">
            <div className="flex items-baseline gap-2.5 flex-wrap">
              <span className="text-3xl font-extrabold text-swift-dark font-mono">
                ₹{product.price.toLocaleString("en-IN")}
              </span>
              {product.mrp > product.price && (
                <>
                  <span className="text-base text-swift-mid line-through font-mono">
            ...
            <p className="text-[10px] text-swift-mid mt-1.5 font-bold uppercase tracking-wider">
                ...
                {variant.options.map((opt) => {
                  ...
                  if (variant.type === "color") {
                    return (
                      ...
                      >
                        <span
                          className="w-4 h-4 rounded-full border border-gray-300 bg-gray-500 shrink-0"
          ...
          <div className="flex items-center gap-4">
            <span className="text-xs font-bold text-swift-dark uppercase tracking-wider">
              Quantity
            </span>
            <div className="flex items-center border border-gray-250 rounded-button bg-white">
              ...
              <button
                onClick={() =>
                  setQuantity((prev) => {
                    if (product.stockCount > 0 && prev >= product.stockCount)
                      return prev;
                    return prev + 1;
                  })
                }
          ...
          <div className="border border-gray-100 rounded-card p-4 bg-gray-50/50 space-y-2">
            <span className="text-xs font-extrabold text-swift-dark uppercase tracking-wide">
              Available Bank Offers
            </span>
            <ul className="space-y-1.5">
              <li className="text-xs text-swift-mid flex items-start gap-1.5">
                <span className="font-bold text-swift-green shrink-0">
                  Offer 1:
                </span>
                <span>
                  Flat 10% Instant Discount on HDFC Bank Credit Cards on orders
                  above ₹5,000.
                </span>
              </li>
              <li className="text-xs text-swift-mid flex items-start gap-1.5">
                ...
                <span>
                  Get 5% unlimited Cashback using SwiftCart Axis Bank cobranded
                  card.
                </span>
              </li>
            </ul>
        ...
        <div className="p-6">
          ...
          {activeTab === "highlights" && (
            <div className="space-y-4">
              <h3 className="font-heading font-bold text-base text-swift-dark">
                Key Highlights
              </h3>
              <ul className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {product.highlights.map((hl, i) => (
                  <li
                    key={i}
                    className="text-sm text-swift-mid flex items-start gap-2"
                  >
              ...
              <table className="w-full text-left border-collapse text-sm">
                <thead>
                  <tr className="border-b border-gray-100 bg-gray-50">
                    <th className="p-3 text-swift-dark font-extrabold">
                ...
                <tbody>
                  {Object.entries(product.specs).map(([key, val]) => (
                    ...
                    >
                      <td className="p-3 font-semibold text-swift-dark shrink-0">
                        {key}
                      </td>
            ...
            <div ref={reviewSectionRef} className="space-y-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-12 gap-8 items-center border-b border-gray-100 pb-8">
                ...
                <div className="md:col-span-4 text-center space-y-2 border-r border-gray-100 py-2">
                  <div className="text-5xl font-extrabold text-swift-dark">
                    {product.rating}
                  </div>
                  <div className="flex items-center justify-center text-swift-orange gap-0.5">
                    {[1, 2, 3, 4, 5].map((s) => (
                      <Star
                        key={s}
                        className={`w-5 h-5 ${s <= Math.round(product.rating) ? "fill-swift-orange" : "text-gray-200"}`}
                      />
                    ))}
                  </div>
                  <p className="text-xs text-swift-mid">
                    Based on {product.reviewCount} user feedback
                  </p>
                </div>

                {/* Bars */}
                <div className="md:col-span-5 space-y-1.5">
                  {Object.keys(starsHistogram)
                    .reverse()
                    .map((stars) => {
                      ...
                      return (
                        <button
                          ...
                          onClick={() =>
                            setReviewsFilter(
                              reviewsFilter === stars ? null : stars,
                            )
                          }
                          className={`w-full flex items-center gap-3 text-left p-1 rounded hover:bg-gray-50 transition-colors ${
                            reviewsFilter === stars
                              ? "bg-swift-orange/5 font-bold"
                              : ""
                          }`}
                        >
                          <span className="w-8 text-xs font-bold text-swift-dark">
                            {stars} ★
                          </span>
                          <div className="flex-grow h-2.5 bg-gray-100 rounded-full overflow-hidden">
                            <div
                              className="h-full bg-swift-orange"
                              style={{ width: `${percent}%` }}
                            />
                          </div>
                          <span className="w-10 text-xs text-swift-mid text-right">
                            {count}
                          </span>
                        </button>
                      );
              ...
              <div className="space-y-6">
                <div className="flex items-center justify-between">
                  <h4 className="font-heading font-extrabold text-sm text-swift-dark">
                    {reviewsFilter
                      ? `${reviewsFilter} Star Feedback`
                      : "Latest Verified Purchases"}
                  </h4>
                  {reviewsFilter && (
                    <button
                ...
                {reviewsToDisplay.length === 0 ? (
                  <p className="text-xs text-swift-mid text-center py-6">
                    No reviews matched this filter selection.
                  </p>
                ) : (
                  reviewsToDisplay.map((rev) => (
                    <div
                      key={rev.id}
                      className="border-b border-gray-50 pb-6 space-y-2"
                    >
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex text-swift-orange gap-0.5">
                          {[1, 2, 3, 4, 5].map((s) => (
                            <Star
                              key={s}
                              className={`w-3.5 h-3.5 ${s <= rev.rating ? "fill-swift-orange" : "text-gray-150"}`}
                            />
                          ))}
                        </div>
                      </div>
                      ...
                      <div className="pl-10 space-y-1">
                        <h5 className="font-bold text-sm text-swift-dark">
                          {rev.title}
                        </h5>
        ...
        <div className="flex flex-col md:flex-row items-center gap-6">
          <div className="flex items-center gap-3 flex-wrap justify-center">
            ...
            <div className="text-center w-24">
              ...
              <span className="block text-[10px] text-swift-dark font-semibold mt-1 truncate">
                {product.name}
              </span>
              <span className="block text-xs font-extrabold text-swift-dark mt-0.5">
                ₹{product.price}
              </span>
            </div>
          ...
          <div className="text-center md:text-left space-y-2">
            <div>
              <div className="text-xs text-swift-mid font-bold">
                Bundle Price:
              </div>
              <div className="flex items-baseline justify-center md:justify-start gap-2">
                <span className="text-xl font-extrabold text-swift-dark font-mono">
                  ₹{(product.price + 899 - 100).toLocaleString("en-IN")}
                </span>
                <span className="text-xs text-swift-mid line-through">
                  ₹{product.price + 899}
                </span>
                <span className="text-xs font-bold text-swift-green bg-swift-green/10 px-1.5 py-0.5 rounded">
                  Save ₹100
                </span>
              </div>
            </div>
              ...
              onClick={() => {
                ...
                const addBuds = async () => {
                  try {
                    ...
                    const buds = response.content?.[0]
                      ? mapBackendProduct(response.content[0])
                      : null;
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-xs flex items-center justify-center p-4 z-50 pointer-events-auto">
          <div className="bg-white rounded-modal w-full max-w-md p-6 shadow-modal overflow-y-auto max-h-full">
            ...
            <form onSubmit={handleReviewSubmit} className="space-y-4">
              {/* Star selector */}
              <div>
                <label className="block text-xs font-bold text-swift-dark mb-1.5">
                  Overall Rating
                </label>
                <div className="flex gap-1.5">
                  {[1, 2, 3, 4, 5].map((s) => (
                    ...
                    >
                      <Star
                        className={`w-8 h-8 ${s <= newReviewRating ? "text-swift-orange fill-swift-orange" : "text-gray-200"}`}
                      />
                    </button>
                  ))}
                </div>
