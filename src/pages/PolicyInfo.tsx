...
import { Shield, Scale, HelpCircle, Truck, RefreshCw, XCircle, AlertCircle, Building2, UserCheck } from 'lucide-react';
...
const POLICIES: { [key: string]: PolicyData } = {
  'track-order': {
            ...
            {data.sections.map((section, idx) => (
              <section key={idx} className="space-y-3">
                ...
                {section.paragraphs.map((p, pIdx) => (
                  <p key={pIdx} className="text-xs sm:text-sm text-swift-mid leading-relaxed whitespace-pre-line">
                    {p}
                  </p>
                ))}
              </section>
