...
import { Trash2, Heart, ShieldCheck, Ticket, ArrowRight, MapPin, ShoppingBag, Minus, Plus, X } from 'lucide-react';
import { useCartStore, useToastStore, useWishlistStore, useAuthStore } from '../store/useSwiftStore';
...
export const Cart: React.FC = () => {
  ...
  const { cart, updateQuantity, removeFromCart, coupon, applyCoupon, removeCoupon, getTotals, selectedAddressId, setSelectedAddressId } = useCartStore();
  ...
  const handleSaveForLater = (productId: string, cartItemId: string) => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 md:px-6 py-6 pb-20">
      ...
      <h2 className="font-heading font-extrabold text-2xl text-swift-dark mb-6">Shopping Cart</h2>
      ...
      {cart.length === 0 ? (
        <div className="bg-white border border-gray-100 rounded-card p-12 text-center max-w-xl mx-auto space-y-4 shadow-card">
          ...
          <div>
            <h3 className="font-heading font-bold text-lg text-swift-dark">Your Shopping Cart is Empty</h3>
            <p className="text-xs text-swift-mid mt-1">Explore daily deals and add products to start shopping.</p>
          </div>
          <Link
            to="/"
              ...
              <div className="bg-white border border-gray-100 rounded-card p-4 shadow-xs flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <div className="flex items-center gap-2">
                  ...
                  <div className="text-left">
                    <span className="text-xs font-extrabold text-swift-dark uppercase tracking-wider block">Deliver to:</span>
                    <span className="text-xs text-swift-mid">
                      {addresses.find(a => a.id === selectedAddressId)?.name || 'Default Address'} - {addresses.find(a => a.id === selectedAddressId)?.addressLine1}, {addresses.find(a => a.id === selectedAddressId)?.city}
                    </span>
                  </div>
                </div>
                <select
                ...
                >
                  {addresses.map(a => (
                    <option key={a.id} value={a.id}>{a.type}: {a.name} ({a.pincode})</option>
                  ))}
                </select>
              </div>
            )}
                  ...
                  >
                    ...
                    <div className="flex-grow flex flex-col justify-between py-1">
                      <div>
                        <div className="flex justify-between items-start gap-4">
                          <h4 className="font-heading font-extrabold text-base text-swift-dark hover:text-swift-orange transition-colors">
                            <Link to={`/product/${item.product.slug || item.product.id}`}>{item.product.name}</Link>
                          </h4>
                          <span className="font-mono font-extrabold text-base text-swift-dark shrink-0">
                            ₹{(item.product.price * item.quantity).toLocaleString('en-IN')}
                          </span>
                        </div>
                        <p className="text-xs text-swift-mid mt-0.5">Sold by: {mockDb.getSellerById(item.product.sellerId)?.name || 'SwiftCart'}</p>
                      ...
                      <div className="flex flex-wrap items-center justify-between gap-4 mt-4 pt-4 border-t border-gray-50">
                        ...
                        <div className="flex items-center gap-3">
                          <button
                            onClick={() => handleSaveForLater(item.product.id, item.cartItemId)}
                          ...
                          >
                            <Heart className={`w-3.5 h-3.5 ${isSaved ? 'fill-swift-blue' : ''}`} />
            ...
            <div className="bg-white border border-gray-100 rounded-card p-4 shadow-card">
              ...
              {coupon ? (
                <div className="bg-swift-green/5 border border-swift-green/20 rounded-button p-3 flex items-center justify-between">
                  <div>
                    <span className="text-xs font-bold text-swift-green uppercase">{coupon.code} Applied</span>
                    <p className="text-[10px] text-swift-mid">{coupon.description}</p>
                  </div>
                  <button
                    onClick={() => {
                      removeCoupon();
                  ...
                  {showCouponFocus && (
                    <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-gray-100 rounded-card shadow-modal z-20 p-2 space-y-2">
                      <div className="flex justify-between items-center text-[10px] font-bold text-swift-mid uppercase tracking-wide border-b border-gray-50 pb-1.5">
                        ...
                        <button type="button" onClick={() => setShowCouponFocus(false)} className="text-swift-red hover:underline">Close</button>
                      </div>
                      {availableCoupons.map(c => (
                        ...
                        >
                          <span className="text-xs font-bold text-swift-blue uppercase">{c.code}</span>
                          <span className="text-[10px] text-swift-mid">{c.description}</span>
                        </button>
                      ))}
                    </div>
                  )}
            ...
            <div className="bg-white border border-gray-100 rounded-card p-6 shadow-card space-y-4">
              ...
              <div className="space-y-2 text-xs font-bold text-swift-mid">
                <div className="flex justify-between">
                  ...
                  <span className="font-mono text-swift-dark">₹{cart.reduce((sum, item) => sum + item.product.mrp * item.quantity, 0).toLocaleString('en-IN')}</span>
                </div>
                <div className="flex justify-between text-swift-green">
                  <span>Discount on MRP</span>
                  <span className="font-mono">-₹{(cart.reduce((sum, item) => sum + item.product.mrp * item.quantity, 0) - totals.subtotal).toLocaleString('en-IN')}</span>
                </div>
                {totals.discount > 0 && (
                  <div className="flex justify-between text-swift-green">
      ...
      <section className="mt-16 space-y-6">
        <h3 className="font-heading font-extrabold text-lg text-swift-dark">Similar Products You Might Like</h3>
