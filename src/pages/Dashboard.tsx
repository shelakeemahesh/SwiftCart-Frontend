...
export const Dashboard: React.FC = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 md:px-6 py-8 pb-20">
      ...
      <div className="flex items-center gap-4 border-b border-gray-150 pb-6 mb-8 text-left">
        {user?.avatarUrl ? (
          <img 
            src={user.avatarUrl} 
            alt={user.name} 
            className="w-14 h-14 rounded-full object-cover shadow-md border-2 border-white ring-2 ring-swift-orange/20" 
            onError={(e) => {
        ...
        <div className={`w-14 h-14 bg-swift-orange text-white rounded-full flex items-center justify-center font-bold text-xl font-heading shadow-md avatar-fallback ${user?.avatarUrl ? 'hidden' : ''}`}>
          {user?.name ? user.name.split(' ').map(n => n[0]).join('') : 'U'}
        </div>
        <div>
          <div className="flex items-center gap-3">
            <h2 className="font-heading font-extrabold text-xl text-swift-dark">{user?.name}</h2>
            {user?.provider && user.provider.toLowerCase() === 'google' && (
              <span className="px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider flex items-center gap-1.5 shadow-sm border bg-red-50 text-red-600 border-red-100">
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-8 items-start">
        ...
        <aside className="md:col-span-3 bg-white border border-gray-100 rounded-card p-4 shadow-card shrink-0">
