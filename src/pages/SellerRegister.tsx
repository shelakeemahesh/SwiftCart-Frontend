...
import {
  ...
  Eye, EyeOff, AlertCircle, Package, TrendingUp, Headphones
} from 'lucide-react';
...
const SELLING_BENEFITS = [
  { icon: Package, title: '45 Cr+ Customers', desc: 'Sell to India\'s largest online marketplace audience' },
  { icon: TrendingUp, title: '7-Day Payments', desc: 'Quick, hassle-free payment settlements every week' },
  { icon: Headphones, title: '24×7 Seller Support', desc: 'Dedicated account manager & support for your store' },
  { icon: Shield, title: 'Secure & Trusted', desc: 'End-to-end secure transactions with buyer protection' },
];
...
function validateStep(step: number, data: FormData): FieldErrors {
  ...
  if (step === 0) {
    ...
    else if (!GSTIN_RE.test(data.gstin.toUpperCase())) e.gstin = 'Invalid GSTIN format (e.g. 22AAAAA0000A1Z5)';
    if (!data.panNumber.trim()) e.panNumber = 'PAN is required';
    else if (!PAN_RE.test(data.panNumber.toUpperCase())) e.panNumber = 'Invalid PAN format (e.g. ABCDE1234F)';
  }
  ...
  if (step === 1) {
    ...
    else if (data.phone.length !== 10) e.phone = 'Enter a valid 10-digit phone number';
    if (!data.password) e.password = 'Password is required';
    else if (data.password.length < 6) e.password = 'Minimum 6 characters required';
  ...
  if (step === 2) {
    ...
    else if (!/^\d{6}$/.test(data.pickupPincode)) e.pickupPincode = 'Enter a valid 6-digit pincode';
    ...
    else if (!IFSC_RE.test(data.ifscCode.toUpperCase())) e.ifscCode = 'Invalid IFSC format (e.g. SBIN0001234)';
  }
...
export const SellerRegister: React.FC = () => {
  ...
  const handleChange = useCallback(
    (field: keyof FormData) => (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
      ...
      // Auto-format
      if (field === 'phone' || field === 'pickupPincode') value = value.replace(/\D/g, '');
      if (field === 'gstin' || field === 'panNumber' || field === 'ifscCode') value = value.toUpperCase();
  ...
  const handleVerifyOtp = async () => {
    ...
    try {
      ...
      addToast(err.message || 'OTP verification failed. Please retry.', 'error');
    } finally {
      setIsSubmitting(false);
    }
  };
      ...
      <div className="min-h-screen bg-swift-bg flex items-center justify-center p-4">
        ...
        <div className="bg-white rounded-modal shadow-modal p-10 text-center max-w-md w-full relative z-10">
          ...
          <p className="text-sm text-swift-mid mb-6">
            Your seller account has been created successfully. You'll be redirected to login shortly.
          </p>
    ...
    <div className="min-h-screen bg-swift-bg flex flex-col lg:flex-row">
      ...
      <div className="hidden lg:flex lg:w-[42%] xl:w-[38%] bg-gradient-to-br from-swift-blue via-[#1a4f8a] to-[#0c3a6b] text-white relative overflow-hidden flex-col justify-between p-10 xl:p-14">
        ...
        <div className="relative z-10">
          <Link to="/" className="inline-flex items-center gap-2 mb-10">
            ...
            <span className="font-heading font-extrabold text-xl tracking-tight">SwiftCart</span>
          </Link>

          <h1 className="font-heading font-extrabold text-3xl xl:text-4xl leading-tight mb-4">
            Start Selling on<br />India's #1 Marketplace
          </h1>
          <p className="text-sm text-blue-200 leading-relaxed max-w-sm">
            Join millions of sellers growing their business on SwiftCart. Register in minutes, start selling today.
          </p>
        </div>
          ...
          {SELLING_BENEFITS.map((b, i) => (
            <div key={i} className="flex items-start gap-4 group">
              ...
              <div>
                ...
                <p className="text-xs text-blue-200 leading-relaxed">{b.desc}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="flex-1 flex flex-col min-h-screen lg:min-h-0">
        ...
        <div className="lg:hidden bg-swift-blue text-white px-5 py-4 flex items-center gap-3">
          ...
          <span className="font-heading font-extrabold text-lg">Seller Registration</span>
        </div>
                ...
                return (
                  <React.Fragment key={i}>
                    <div className="flex flex-col items-center gap-1.5 relative">
                      ...
                      >
                        {isCompleted ? <Check className="w-5 h-5" strokeWidth={3} /> : <StepIcon className="w-4 h-4" />}
                      </div>
                      <span
                        className={`text-[10px] font-bold text-center hidden sm:block transition-colors duration-300 ${
                          isCurrent ? 'text-swift-dark' : isCompleted ? 'text-swift-green' : 'text-swift-mid'
                        }`}
                      >
                        {s.label}
                      </span>
          ...
          <div className="max-w-xl mx-auto">
              ...
              >
                ...
                {step === 0 && (
                  <div className="space-y-5">
                    <div>
                      <label htmlFor="seller-reg-business-name" className={labelCls}>
                    ...
                    <div>
                      ...
                      />
                      {renderError('gstin')}
                      <p className="text-[10px] text-swift-mid mt-1">15-character GST Identification Number</p>
                    </div>
                ...
                {step === 1 && (
                  <div className="space-y-5">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                      <div>
                        <label htmlFor="seller-reg-password" className={labelCls}>
                      ...
                      <div>
                        <label htmlFor="seller-reg-confirm-password" className={labelCls}>
                          Confirm Password <span className="text-swift-red">*</span>
                        </label>
                        <div className="relative">
                          <input
                ...
                {step === 2 && (
                  <div className="space-y-5">
                    <div>
                      <label htmlFor="seller-reg-pickup-address" className={labelCls}>
                      ...
                      <textarea
                        ...
                        onChange={handleChange('pickupAddress') as any}
                    ...
                    <div className="border-t border-gray-100 pt-5">
                      ...
                      <div className="space-y-5">
                        <div>
                          <label htmlFor="seller-reg-bank-account" className={labelCls}>
                            Account Number <span className="text-swift-red">*</span>
                          </label>
                          <input
                    ...
                    {!otpSent ? (
                      <>
                        <div className="space-y-4">
                          <SummarySection
                            ...
                            icon={<Store className="w-4 h-4 text-swift-orange" />}
                            items={[
                          ...
                          <SummarySection
                            title="Pickup & Bank"
                            icon={<MapPin className="w-4 h-4 text-swift-green" />}
                            items={[
                              { label: 'Pickup Address', value: form.pickupAddress },
                              { label: 'Pincode', value: form.pickupPincode },
                              { label: 'Account No.', value: form.bankAccountNumber.replace(/.(?=.{4})/g, '•') },
                      ...
                      <div className="space-y-6">
                        <div className="text-center">
                          ...
                          <h3 className="font-heading font-bold text-lg text-swift-dark">Verify Your Phone</h3>
                          <p className="text-xs text-swift-mid mt-1">
                            Enter the 6-digit OTP sent to <span className="font-bold text-swift-dark">+91 {form.phone}</span>
                          </p>
                        </div>
                        ...
                        <div className="flex justify-center">
                          {otpTimer > 0 ? (
                            <span className="text-xs text-swift-mid">
                              Resend OTP in <span className="font-bold font-mono text-swift-dark">{otpTimer}s</span>
                            </span>
                          ) : (
                            <button
                              type="button"
            ...
            <div className="mt-8 text-center space-y-3">
              <p className="text-xs text-swift-mid">
                ...
                <Link to="/login" id="seller-reg-login-link" className="text-swift-blue font-bold hover:underline">
                  Login here
                </Link>
              </p>
...
const SummarySection: React.FC<SummarySectionProps> = ({ title, icon, items }) => (
  <div className="bg-swift-bg/60 border border-gray-100 rounded-card p-4">
    <div className="flex items-center gap-2 mb-3">
      <div className="w-7 h-7 rounded-full bg-white shadow-sm flex items-center justify-center">{icon}</div>
      <h4 className="font-heading font-bold text-sm text-swift-dark">{title}</h4>
    </div>
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-2">
      {items.map((item, i) => (
        <div key={i} className="flex flex-col">
          <span className="text-[10px] text-swift-mid font-semibold uppercase tracking-wide">{item.label}</span>
          <span className="text-sm text-swift-dark font-medium truncate">{item.value || '—'}</span>
        </div>
      ))}
    </div>
  </div>
