...
const API_PRESETS = [
  {
    ...
    description:
      "Retrieve summary of total revenues, active orders, and sales counts.",
  },
  {
  ...
  },
  {
    ...
    description:
      "Approve a product to make it active and visible in catalog searches.",
  },
  {
    name: "Reject Product",
    method: "PUT",
  ...
  },
  {
    ...
    description:
      "Fetch paginated list of all system orders sorted descending by ID.",
    defaultParams: [
      { key: "page", value: "0" },
      { key: "size", value: "20" },
    ],
  ...
  },
  {
    ...
    description:
      "Trigger asynchronous background bulk import of products using a CSV file.",
    requiresFile: true,
  },
  {
    ...
    description:
      "Poll status (PENDING, SUCCESS, FAILED) of an asynchronous CSV import job.",
  },
];
...
export const AdminPanel = () => {
  ...
  const selectedPreset =
    API_PRESETS.find((p) => p.name === selectedPresetName) || API_PRESETS[0];
  const [apiUrl, setApiUrl] = useState(API_BASE_URL);
  const [apiPath, setApiPath] = useState(API_PRESETS[0].path);
  const [apiMethod, setApiMethod] = useState(API_PRESETS[0].method);
  ...
  const [mockFlashSales, setMockFlashSales] = useState([]);
    ...
    setMockCoupons([
      {
        ...
        value: 20,
        minSpend: 999,
        description: "20% Off on minimum spend of ₹999",
        active: true,
      },
      {
        id: 2,
      ...
      },
      {
        ...
        value: 0,
        minSpend: 499,
        description: "Free Delivery on order value ₹499+",
        active: true,
      },
    ]);
    setMockFlashSales([]);
  ...
  // Mock changes
  const handleToggleMockRole = async (id, currentRole) => {
    const nextRole =
      currentRole === "ADMIN"
        ? "CUSTOMER"
  ...
  const handleToggleMockVerify = async (id) => {
    const targetUser = mockUsers.find((u) => u.id === id);
    if (!targetUser) return;
    const nextVerifiedState = !targetUser.verified;
    try {
      await apiClient.put(
        `/api/v1/admin/users/${id}/verify?verified=${nextVerifiedState}`,
      );
      setMockUsers((prev) =>
        prev.map((u) =>
          u.id === id ? { ...u, verified: nextVerifiedState } : u,
        ),
      );
      addToast(
        `User ${nextVerifiedState ? "activated" : "deactivated"} in database`,
        "success",
      );
    } catch (err) {
      setMockUsers((prev) =>
  ...
  const handleApproveProductMock = (id, approve) => {
    setMockProducts((prev) =>
      prev.map((p) => (p.id === id ? { ...p, inStock: approve } : p)),
    );
    addToast(
      `[MOCK] Product ${approve ? "approved" : "rejected"} successfully.`,
      approve ? "success" : "error",
    );
  };

  const handleCreateMockCoupon = (e) => {
    e.preventDefault();
  ...
  const handleDeactivateMockCoupon = (id) => {
    setMockCoupons((prev) =>
      ...
      const interval = setInterval(async () => {
        try {
          const statusRes = await apiClient.get(
            `/api/v1/admin/products/import/status/${jobId}`,
          );
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-gray-100 pb-6 mb-8">
        <div>
          ...
          <p className="text-sm text-swift-mid mt-1">
            Manage customer catalog moderation, coupon overrides, system
            telemetry, and test live server APIs.
          </p>
        </div>
        ...
        <div className="bg-swift-bg border border-gray-250/70 p-3 rounded-card flex flex-col gap-1.5 self-start">
          <span className="text-[10px] uppercase font-bold tracking-wider text-swift-mid">
          ...
          <div className="flex gap-2">
            <button
              onClick={() => {
                setRole("ADMIN");
                addToast("Role toggled to ADMIN", "info");
              }}
            ...
            <button
              onClick={() => {
                ...
                setTimeout(() => {
                  setRole("CUSTOMER");
                  addToast(
                    "Role toggled to CUSTOMER. Panel exited.",
                    "warning",
                  );
                }, 50);
              }}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
        ...
        <div className="hidden lg:block lg:col-span-1 space-y-4">
          <div className="bg-white border border-gray-100 rounded-card p-4 shadow-sm space-y-1">
            <div className="text-[10px] uppercase font-bold text-swift-mid tracking-wider px-3 mb-2">
              Management Tabs
            </div>
          ...
          <div className="bg-white border border-gray-100 rounded-card p-4 shadow-sm text-xs space-y-2">
            <span className="font-bold text-swift-dark block text-[10px] uppercase tracking-wider text-swift-mid">
              Services Status
            </span>
        ...
        <div className="lg:col-span-3 space-y-6">
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="bg-white border border-gray-100 rounded-card p-5 shadow-sm space-y-2 relative overflow-hidden group">
                  ...
                  <span className="text-xs uppercase font-bold text-swift-mid tracking-wider block">
                    Total Sales Revenue
                  </span>
                  <h3 className="font-heading font-extrabold text-3xl text-swift-dark font-mono">
                    ₹4,89,120
                  </h3>
                ...
                <div className="bg-white border border-gray-100 rounded-card p-5 shadow-sm space-y-2 relative overflow-hidden group">
                  ...
                  <span className="text-[10px] text-swift-mid font-semibold block">
                    COD: 9 • Razorpay Prepaid: 5
                  </span>
                </div>
                ...
                <div className="bg-white border border-gray-100 rounded-card p-5 shadow-sm space-y-2 relative overflow-hidden group">
                  ...
                  <span className="text-[10px] text-swift-green font-bold block">
                    158 verified today
                  </span>
                </div>
              </div>
              ...
              <div className="bg-white border border-gray-100 rounded-card p-6 shadow-sm space-y-4">
                <div className="flex items-center justify-between border-b border-gray-50 pb-4">
                  ...
                  <span className="text-xs font-semibold text-swift-mid">
                    Telemetry from Elasticsearch/MySQL DB
                  </span>
                </div>
                  ...
                  <table className="w-full text-left text-xs border-collapse">
                    ...
                    <tbody className="divide-y divide-gray-100 font-semibold text-swift-dark">
                      {mockProducts.slice(0, 5).map((p, idx) => (
                        <tr key={p.id}>
                          <td className="py-3 font-mono font-bold text-swift-blue">
                            {p.id}
                          </td>
                          ...
                          <td className="py-3 text-center font-mono text-swift-blue">
                            {140 - idx * 24}
                          </td>
                          <td className="py-3 text-right font-mono text-swift-green">
                            ₹
                            {(p.price * (140 - idx * 24)).toLocaleString(
                              "en-IN",
                            )}
                          </td>
                        </tr>
              ...
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 border-b border-gray-50 pb-4">
                <div>
                  ...
                  <p className="text-xs text-swift-mid">
                    Promote authorization levels or deactivate users simulator.
                  </p>
                </div>
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-left text-xs border-collapse">
                  ...
                  <tbody className="divide-y divide-gray-100 font-semibold text-swift-dark">
                    {mockUsers.map((u) => (
                      <tr key={u.id}>
                        <td className="py-3 font-mono text-swift-mid">
                          {u.id}
                        </td>
                        <td className="py-3">
                          <div className="font-bold">{u.name}</div>
                          <div className="text-[10px] text-swift-mid leading-none font-normal">
                            {u.email}
                          </div>
                        </td>
                        ...
                        <td className="py-3">
                          <span
                            className={`px-2 py-0.5 rounded text-[10px] font-bold ${u.verified ? "bg-swift-green/10 text-swift-green" : "bg-swift-red/10 text-swift-red"}`}
                          >
          ...
          {activeTab === "products" && (
            <div className="bg-white border border-gray-100 rounded-card p-6 shadow-sm space-y-4">
              <div className="border-b border-gray-50 pb-4">
                <h3 className="font-heading font-extrabold text-lg text-swift-dark">
                  Catalog Moderation Dashboard
                </h3>
                <p className="text-xs text-swift-mid">
                  Approve or reject newly imported catalog products.
                </p>
              </div>
                  ...
                  <tbody className="divide-y divide-gray-100 font-semibold text-swift-dark">
                    {mockProducts.slice(0, 6).map((p) => (
                      <tr key={p.id}>
                        ...
                        <td className="py-3">
                          <span
                            className={`px-2 py-0.5 rounded text-[10px] font-bold ${p.inStock ? "bg-swift-green/10 text-swift-green" : "bg-amber-500/10 text-amber-500"}`}
                          >
                        ...
                        <td className="py-3 text-right space-x-1.5">
                          {!p.inStock ? (
                            <>
                              <button
                                onClick={() =>
                                  handleApproveProductMock(p.id, true)
                                }
                              ...
                              <button
                                onClick={() =>
                                  handleApproveProductMock(p.id, false)
                                }
            ...
            <div className="space-y-6">
              ...
              <div className="bg-white border border-gray-100 rounded-card p-6 shadow-sm">
                <div className="border-b border-gray-50 pb-4 mb-4">
                  <h3 className="font-heading font-extrabold text-lg text-swift-dark">
                    Generate System Coupon Code
                  </h3>
                  <p className="text-xs text-swift-mid">
                    Create discount codes applicable at cart checkout.
                  </p>
                </div>

                <form
                  onSubmit={handleCreateMockCoupon}
                  className="grid grid-cols-1 md:grid-cols-4 gap-4"
                >
                  <div>
                    <label className="block text-[10px] font-bold text-swift-dark uppercase mb-1">
                      Coupon Code *
                    </label>
                    <input
                      type="text"
                    ...
                    <tbody className="divide-y divide-gray-100 font-semibold text-swift-dark">
                      {mockCoupons.map((c) => (
                        <tr key={c.id}>
                          ...
                          <td className="py-3 font-bold text-swift-orange font-mono">
                            {c.code}
                          </td>
                          ...
                          <td className="py-3">
                            <span
                              className={`px-2 py-0.5 rounded text-[10px] font-bold ${c.active ? "bg-swift-green/10 text-swift-green" : "bg-swift-red/10 text-swift-red"}`}
                            >
          ...
          {activeTab === "flash" && (
            <div className="space-y-6">
              ...
              <div className="bg-white border border-gray-100 rounded-card p-6 shadow-sm">
                <div className="border-b border-gray-50 pb-4 mb-4">
                  <h3 className="font-heading font-extrabold text-lg text-swift-dark">
                    Schedule Product Flash Sale
                  </h3>
                  <p className="text-xs text-swift-mid">
                    Override a product base price during specific temporal
                    windows.
                  </p>
                </div>

                <form
                  onSubmit={handleCreateMockFlash}
              ...
              <div className="bg-white border border-gray-100 rounded-card p-6 shadow-sm space-y-4">
                <div className="border-b border-gray-50 pb-4">
                  <h3 className="font-heading font-extrabold text-lg text-swift-dark">
                    Configured Flash Sales Scheduler
                  </h3>
                </div>
                    ...
                    <table className="w-full text-left text-xs border-collapse">
                      <thead>
                        <tr className="border-b border-gray-150 text-swift-mid font-bold">
                          ...
                          <th className="py-2.5 font-mono text-swift-red">
                            Flash Price
                          </th>
                      ...
                      <tbody className="divide-y divide-gray-100 font-semibold text-swift-dark">
                        {mockFlashSales.map((f) => (
                          <tr key={f.id}>
                            ...
                            <td className="py-3 font-mono text-swift-red">
                              ₹{f.salePrice}
                            </td>
                            <td className="py-3 font-mono">
                              {f.stockLimit} items
          ...
          {activeTab === "csv" && (
            <div className="bg-white border border-gray-100 rounded-card p-6 shadow-sm space-y-6">
              <div className="border-b border-gray-50 pb-4">
                <h3 className="font-heading font-extrabold text-lg text-swift-dark">
                  Asynchronous CSV Bulk Importer
                </h3>
                <p className="text-xs text-swift-mid">
                  Upload database items in bulk. Handled asynchronously via
                  Spring ProductService threading.
                </p>
              </div>
              ...
              <form onSubmit={handleCsvImport} className="space-y-4">
                <div className="border-2 border-dashed border-gray-250 hover:border-swift-orange rounded-card p-8 text-center transition-all bg-swift-bg/20">
                  ...
                  <p className="text-sm font-bold text-swift-dark mb-1">
                    Choose products metadata CSV file
                  </p>
                  <p className="text-xs text-swift-mid mb-4">
                    File format must match header attributes (name, price,
                    stock, brand, etc.)
                  </p>
                ...
                <div className="flex justify-between items-center">
                  <a
                    ...
                    onClick={(e) => {
                      e.preventDefault();
                      const csvContent =
                        "data:text/csv;charset=utf-8,brand,name,description,price,mrp,stockQty,categoryId,sellerId\nSwiftBrand,Smart Glass Pro,High end AR visual wear glass,12999.00,19999.00,45,1,2\n";
                      const encodedUri = encodeURI(csvContent);
                      const link = document.createElement("a");
                      link.setAttribute("href", encodedUri);
                      link.setAttribute(
                        "download",
                        "swiftcart_import_template.csv",
                      );
              ...
              {importJobId && (
                <div className="bg-swift-bg rounded-card border border-gray-200 p-5 space-y-3">
                  <div className="flex items-center justify-between text-xs">
                    <span className="font-bold text-swift-mid uppercase tracking-wide">
                      Import Telemetry Monitor
                    </span>
                    <span className="font-mono bg-gray-100 px-2 py-0.5 rounded font-bold text-swift-dark">
                      Job ID: {importJobId}
                    </span>
                  </div>
                    ...
                    <div>
                      ...
                      <p className="text-xs text-swift-mid">
                        {importStatus === "PENDING" &&
                          "Worker thread spinning up in background..."}
                        {importStatus === "PROCESSING" &&
                          "Importing row data, indexing search cluster..."}
                        {importStatus === "COMPLETED" &&
                          "Successfully completed! Products added to MySQL and Elasticsearch."}
                      </p>
                    </div>
                  </div>
                </div>
              ...
              <div className="bg-white border border-gray-100 rounded-card p-6 shadow-sm space-y-4">
                <div className="border-b border-gray-50 pb-4">
                  ...
                  <p className="text-xs text-swift-mid mt-1">
                    Send real REST API requests directly to the running Spring
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    >
                      {API_PRESETS.map((p) => (
                        <option key={p.name} value={p.name}>
                          {p.method} {p.path} — {p.name}
                        </option>
                      ))}
                    </select>
                    <p className="text-[10px] text-swift-mid mt-1 font-semibold italic">
                  ...
                  <div className="md:col-span-3 bg-swift-bg/30 p-3 rounded-card border border-gray-200">
                    <label className="block text-[10px] font-bold text-swift-dark uppercase mb-1 flex items-center justify-between">
                      ...
                      <span className="text-[9px] text-swift-mid uppercase font-bold">
                        Injected in Headers
                      </span>
                    </label>
                    <div className="flex gap-2">
                ...
                <div className="border-t border-gray-150 pt-4 space-y-3 text-left">
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center justify-between">
                      <span className="text-[10px] font-bold text-swift-dark uppercase">
                        Query Parameters
                      </span>
                      <button
                        ...
                        {queryParams.map((p, idx) => (
                          <div key={idx} className="flex gap-2 items-center">
                            <input
                              ...
                              value={p.key}
                              onChange={(e) =>
                                updateQueryParam(idx, "key", e.target.value)
                              }
                            ...
                            <input
                              ...
                              value={p.value}
                              onChange={(e) =>
                                updateQueryParam(idx, "value", e.target.value)
                              }
                  ...
                  {["POST", "PUT"].includes(apiMethod) &&
                    !selectedPreset.requiresFile && (
                      <div className="space-y-1.5 pt-2">
                        <span className="text-[10px] font-bold text-swift-dark uppercase block">
                  ...
                  {selectedPreset.requiresFile && (
                    <div className="space-y-1.5 pt-2 bg-swift-bg/20 p-3 rounded border border-dashed border-gray-200 text-center">
                      <span className="text-[10px] font-bold text-swift-dark uppercase block mb-1">
                ...
                <div className="overflow-auto max-h-96">
                  {responseBody ? (
                    ...
                    <pre className="text-xs font-mono text-zinc-500 font-normal">
                      No request dispatched yet. Choose a preset template above
                      and click "Execute HTTP Request" to capture live
                      telemetry.
                    </pre>
                  )}
                </div>
