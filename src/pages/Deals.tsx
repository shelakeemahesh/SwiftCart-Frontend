...
export const Deals: React.FC = () => {
  const [dealCategory, setDealCategory] = useState<'All' | 'Electronics' | 'Fashion' | 'Home'>('All');

  // Big Sale countdown timer
  const [timeLeft, setTimeLeft] = useState({ days: 2, hours: 14, minutes: 52, seconds: 12 });
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 md:px-6 py-6 pb-20 space-y-10 text-left">
      ...
      <section className="bg-gradient-to-r from-[#D85A30] to-[#EF9F27] rounded-card overflow-hidden relative shadow-card text-white p-6 sm:p-10 flex flex-col md:flex-row items-center justify-between gap-6">
        ...
        <div className="space-y-3 sm:space-y-4 max-w-xl">
          ...
          <p className="text-xs sm:text-sm opacity-90 leading-relaxed font-medium">
            Get highest price cuts on popular brands in electronics, casual streetwear clothing, and dynamic house kitchen appliances.
          </p>
        </div>
        ...
        <div className="bg-white/10 backdrop-blur-md rounded-card border border-white/20 p-5 text-center min-w-[280px]">
          ...
          <div className="flex justify-center gap-4 font-mono font-extrabold">
            <div>
              <span className="block text-2xl sm:text-3xl">{timeLeft.days.toString().padStart(2, '0')}</span>
              <span className="text-[9px] uppercase tracking-wider text-white/70 block mt-0.5">Days</span>
            </div>
            <span className="text-xl sm:text-2xl mt-1">:</span>
            <div>
              <span className="block text-2xl sm:text-3xl">{timeLeft.hours.toString().padStart(2, '0')}</span>
              <span className="text-[9px] uppercase tracking-wider text-white/70 block mt-0.5">Hours</span>
            </div>
            <span className="text-xl sm:text-2xl mt-1">:</span>
            <div>
              <span className="block text-2xl sm:text-3xl">{timeLeft.minutes.toString().padStart(2, '0')}</span>
              <span className="text-[9px] uppercase tracking-wider text-white/70 block mt-0.5">Mins</span>
            </div>
            <span className="text-xl sm:text-2xl mt-1">:</span>
            <div>
              <span className="block text-2xl sm:text-3xl">{timeLeft.seconds.toString().padStart(2, '0')}</span>
              <span className="text-[9px] uppercase tracking-wider text-white/70 block mt-0.5">Secs</span>
            </div>
          </div>
        </div>
      ...
      <section className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {[
          { label: '40% - 70% Off', desc: 'Highest discount drops on selected cards', icon: <Percent className="w-5 h-5" /> },
          { label: '100% Verified Deals', desc: 'Sourced from authorized brand merchants', icon: <Award className="w-5 h-5" /> },
          { label: 'Free Delivery', desc: 'Applicable on all deal items above ₹499', icon: <Flame className="w-5 h-5" /> }
        ].map((item, idx) => (
          <div key={idx} className="border border-gray-150 rounded-card p-4 flex items-center gap-3 bg-white shadow-xs">
            <span className="p-2.5 bg-swift-orange/10 rounded-full text-swift-orange shrink-0">
              {item.icon}
            </span>
            <div>
              <div className="text-xs font-extrabold text-swift-dark">{item.label}</div>
      ...
      <section className="space-y-6">
        <div className="flex flex-col sm:flex-row justify-between items-baseline gap-4 border-b border-gray-100 pb-3">
          <h3 className="font-heading font-extrabold text-lg text-swift-dark">Trending Offers Catalog</h3>
          
          <div className="flex gap-1.5 flex-wrap">
            {(['All', 'Electronics', 'Fashion', 'Home'] as const).map(cat => (
              <button
                key={cat}
        ...
        {dealProducts.length === 0 ? (
          <div className="bg-white border border-gray-100 rounded-card p-12 text-center text-swift-mid">
            No deal items matching this category at this moment. Please check back later.
          </div>
        ) : (
