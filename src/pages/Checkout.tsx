...
export const Checkout: React.FC = () => {
  ...
  const { cart, getTotals, selectedAddressId, setSelectedAddressId, clearCart } = useCartStore();
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 md:px-6 py-8 pb-20">
          ...
          {[
            ...
            return (
              <div key={s.num} className="flex flex-col items-center">
                ...
                <span className={`text-[10px] sm:text-xs font-bold mt-2 uppercase tracking-wide ${
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className={`${step === 4 ? 'lg:col-span-12' : 'lg:col-span-8'} bg-white border border-gray-100 rounded-card p-6 shadow-card`}>
          ...
          {step === 1 && (
            <div className="space-y-6">
              ...
              {showAddressForm ? (
                // Add Address Form
                <form onSubmit={handleAddAddressSubmit} className="space-y-4 border border-gray-150 rounded-card p-4 bg-gray-50/50">
                  <span className="block text-xs font-extrabold text-swift-dark uppercase tracking-wider">New Address Form</span>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-xs font-bold text-swift-dark mb-1">Full Name *</label>
                      <input
                        type="text"
                        value={addrName}
                    ...
                    <div>
                      <label className="block text-xs font-bold text-swift-dark mb-1">Pincode *</label>
                      <input
                        type="text"
                        maxLength={6}
                        value={addrPincode}
                    ...
                    <div>
                      <label className="block text-xs font-bold text-swift-dark mb-1">Flat, House No., Building *</label>
                      <input
                        type="text"
                        value={addrLine1}
                    ...
                    <div>
                      <label className="block text-xs font-bold text-swift-dark mb-1">Area, Street, Sector</label>
                      <input
                        type="text"
                        value={addrLine2}
                    ...
                    <div>
                      <label className="block text-xs font-bold text-swift-dark mb-1">City *</label>
                      <input
                        type="text"
                        value={addrCity}
                    ...
                    <div>
                      <label className="block text-xs font-bold text-swift-dark mb-1">State *</label>
                      <input
                        type="text"
                        value={addrState}
                    ...
                    <div>
                      <label className="block text-xs font-bold text-swift-dark mb-1">Address Type</label>
                      <select
                        value={addrType}
                ...
                <div className="space-y-3">
                  ...
                  {addresses.length === 0 && (
                    <p className="text-center text-xs text-swift-mid py-4">No saved addresses found. Please add a new one.</p>
                  )}
            ...
            <div className="space-y-6">
                ...
                {cart.map(item => (
                  <div key={item.cartItemId} className="flex gap-4 border-b border-gray-50 pb-4">
                    ...
                    <div className="flex-grow text-left">
                      <h4 className="text-sm font-bold text-swift-dark truncate">{item.product.name}</h4>
                      <p className="text-xs text-swift-mid">Quantity: {item.quantity} | Seller: {mockDb.getSellerById(item.product.sellerId)?.name}</p>
                    ...
                    <div className="text-right">
                      <span className="text-sm font-bold text-swift-dark">₹{(item.product.price * item.quantity).toLocaleString('en-IN')}</span>
                      <p className="text-[10px] text-swift-green font-bold flex items-center gap-1 mt-1">
                        <Calendar className="w-3.5 h-3.5" />
                        <span>Delivery in {item.product.deliveryDays} Days</span>
                      </p>
                    </div>
                  </div>
                ))}
              ...
              {activeAddress && (
                <div className="bg-gray-50 p-4 border border-gray-100 rounded-card text-left">
                  <span className="text-[10px] font-bold text-swift-mid uppercase tracking-wide block">Shipment delivery address:</span>
                  <div className="text-xs font-bold text-swift-dark mt-1">{activeAddress.name} ({activeAddress.type})</div>
                  <p className="text-xs text-swift-mid mt-0.5">{activeAddress.addressLine1}, {activeAddress.city} - {activeAddress.pincode}</p>
                </div>
              )}
                ...
                <div className="sm:col-span-8">
                  ...
                  {paymentTab === 'card' && (
                    <form onSubmit={handlePaymentSubmit} className="space-y-4 text-left">
                      <div>
                        <label className="block text-xs font-bold text-swift-dark mb-1">Cardholder Name *</label>
                        <input
                          type="text"
                          value={cardName}
                      ...
                      <div>
                        <label className="block text-xs font-bold text-swift-dark mb-1">Card Number *</label>
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          <label className="block text-xs font-bold text-swift-dark mb-1">Expiry *</label>
                          <input
                            type="text"
                            value={cardExpiry}
                        ...
                        <div>
                          <label className="block text-xs font-bold text-swift-dark mb-1">CVV *</label>
                          <input
                            type="password"
                            maxLength={3}
                            value={cardCvv}
                      ...
                      >
                        Pay ₹{(totals.total + 10).toLocaleString('en-IN')} Secured
                      </button>
                    </form>
                  )}
                  ...
                  {paymentTab === 'upi' && (
                    <form onSubmit={handlePaymentSubmit} className="space-y-4 text-center">
                      ...
                      <p className="text-[10px] text-swift-mid leading-relaxed">
                        Scan this dynamic QR code using GPay, PhonePe, or BHIM UPI apps to complete payment instantly.
                      </p>

                      <div className="text-swift-mid font-bold text-xs uppercase my-2">— OR enter UPI ID —</div>
                      ...
                      >
                        Authorize & Pay ₹{(totals.total + 10).toLocaleString('en-IN')}
                      </button>
                    </form>
                  )}
                  ...
                  {paymentTab === 'cod' && (
                    <form onSubmit={handlePaymentSubmit} className="space-y-4 text-center p-4 bg-gray-50 border border-gray-150 rounded-card">
                      <div className="text-left space-y-2">
                        <span className="block text-xs font-bold text-swift-dark uppercase tracking-wider">Cash on Delivery Terms</span>
                        <p className="text-xs text-swift-mid leading-relaxed">
                          Pay by cash, UPI, or card on the spot when your packages are delivered to your doorstep. An extra ₹15 service fee applies for Cash orders (waived for you).
                        </p>
                      </div>
                      ...
                      <button
                        ...
                        className="w-full py-3.5 bg-swift-orange hover:bg-swift-orange-hover text-white rounded-button font-bold text-sm shadow-sm transition-all mt-4"
                      >
                        Confirm COD Order (₹{(totals.total + 10).toLocaleString('en-IN')})
                      </button>
                    </form>
                  )}
          ...
          {step === 4 && confirmedOrder && (
            <div className="text-center py-10 space-y-6">
              ...
              <div className="space-y-2">
                <h3 className="font-heading font-extrabold text-2xl text-swift-dark">Order Confirmed!</h3>
                <p className="text-xs text-swift-mid max-w-sm mx-auto leading-relaxed">
                  Thank you for shopping with SwiftCart! We have received your payment and your shipment logistics are being configured.
                </p>
              </div>
              ...
              <div className="max-w-md mx-auto bg-gray-50 border border-gray-150 rounded-card p-4 text-left space-y-3 font-semibold text-xs text-swift-mid">
                <div className="flex justify-between border-b border-gray-200 pb-2">
                  ...
                  <span className="font-mono text-swift-dark font-bold text-sm">{confirmedOrder.id}</span>
                </div>
                <div className="flex justify-between border-b border-gray-200 pb-2">
                ...
                <div className="flex justify-between border-b border-gray-200 pb-2">
                  ...
                  <span className="text-swift-dark">{confirmedOrder.deliveryAddress.type} ({confirmedOrder.deliveryAddress.name})</span>
                </div>
                <div className="flex justify-between">
                  <span>Payment Gateway:</span>
                  <span className="text-swift-dark uppercase">{confirmedOrder.paymentMethod}</span>
                </div>
              </div>

              {/* Email advisory */}
              <p className="text-[10px] text-swift-mid">
                A verification invoice receipt has been dispatched to <span className="font-bold text-swift-dark">{user?.email}</span>.
              </p>
