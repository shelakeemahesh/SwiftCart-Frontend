...
export function mapBackendCartItem(item: any): CartItem {
  ...
  const selectedVariant: { [key: string]: string } = {};
  if (item.variant) {
...
export function mapBackendOrder(o: any): Order {
  ...
  const items = Array.isArray(o.items)
    ? o.items.map((item: any) => {
        ...
        return {
          ...
          price: item.unitPrice !== undefined ? Number(item.unitPrice) : Number(item.price || 0),
          quantity: item.quantity,
          image: productSnapshot.imageUrl || (item.product?.images && item.product.images[0]?.imageUrl) || '',
          selectedVariant: item.variant ? { [item.variant.variantKey]: item.variant.variantVal } : undefined
        };
      })
    : [];
  ...
  const mappedAddress = o.address ? {
    ...
    type: (o.address.label === 'HOME' ? 'Home' : o.address.label === 'WORK' ? 'Work' : 'Other') as 'Home' | 'Work' | 'Other',
    isDefault: o.address.default
  } : {
    id: '', name: '', phone: '', pincode: '', addressLine1: '', addressLine2: '', city: '', state: '', type: 'Home' as 'Home' | 'Work' | 'Other'
  };

  const today = o.placedAt ? o.placedAt.split('T')[0] : new Date().toISOString().split('T')[0];
  ...
  return {
    ...
    status: o.status === 'DELIVERED' ? 'Delivered' : 
            ...
            o.status === 'SHIPPED' ? 'Dispatched' : 'Ordered',
    items,
    subtotal: o.mrpTotal !== undefined ? Number(o.mrpTotal) : 0,
    discount: (o.discountTotal !== undefined ? Number(o.discountTotal) : 0) + (o.couponDiscount !== undefined ? Number(o.couponDiscount) : 0),
    ...
    trackingTimeline: [
      { title: 'Ordered', description: 'Order successfully placed on SwiftCart', date: `${today} 14:40`, completed: true },
      { title: 'Dispatched', description: 'Awaiting shipping courier confirmation', date: 'Pending', completed: o.status === 'SHIPPED' || o.status === 'DELIVERED' },
      { title: 'Out for Delivery', description: 'Pending dispatched courier', date: 'Pending', completed: o.status === 'DELIVERED' },
      { title: 'Delivered', description: 'Package has been delivered', date: 'Pending', completed: o.status === 'DELIVERED' }
    ]
  };
}
...
interface AuthStore {
  ...
  isLoggedIn: boolean;
  addresses: Address[];
  login: (phone: string, name?: string, role?: 'CUSTOMER' | 'SELLER' | 'ADMIN') => void;
  loginWithAuthData: (authData: any) => void;
  logout: () => void;
...
export const useAuthStore = create<AuthStore>((set, get) => ({
  user: typeof window !== 'undefined' && localStorage.getItem('sc_logged_in') === 'true'
    ? { 
        name: localStorage.getItem('sc_user_name') || 'John Doe', 
        phone: localStorage.getItem('sc_user_phone') || '9876543210', 
        email: localStorage.getItem('sc_user_email') || 'mahesh@swiftcart.com',
        role: (localStorage.getItem('sc_user_role') as 'CUSTOMER' | 'SELLER' | 'ADMIN') || 'ADMIN',
        provider: localStorage.getItem('sc_user_provider') || 'LOCAL',
        avatarUrl: localStorage.getItem('sc_user_avatar_url') || ''
      }
    : null,
  isLoggedIn: typeof window !== 'undefined' && localStorage.getItem('sc_logged_in') === 'true',
  addresses: [],
  ...
  login: (phone, name = 'Mahesh Kumar', role = 'ADMIN') => {
    ...
    set({
      isLoggedIn: true,
      user: { name, phone, email: 'mahesh@swiftcart.com', role, provider: 'LOCAL', avatarUrl: '' }
    });
  },

  loginWithAuthData: (authData: any) => {
    localStorage.setItem('sc_logged_in', 'true');
  ...
  fetchAddresses: async () => {
    try {
      ...
      const mapped = dbAddresses.map((a: any) => ({
        id: String(a.id),
        ...
        city: a.city,
        state: a.state,
        type: a.label === 'HOME' ? 'Home' : a.label === 'WORK' ? 'Work' : 'Other',
        isDefault: a.default
      }));
...
interface CartStore {
  ...
  addToCart: (product: Product, quantity: number, selectedVariant: { [key: string]: string }) => Promise<void>;
...
export const useCartStore = create<CartStore>((set, get) => ({
  cart: typeof window !== 'undefined' ? JSON.parse(localStorage.getItem('sc_cart_items') || '[]') : [],
  isCartOpen: false,
  coupon: null,
  ...
  addToCart: async (product, quantity, selectedVariant) => {
    ...
    if (isLoggedIn) {
      try {
        ...
        if (product.variants && product.variants.length > 0) {
          const matching = product.variants.find((v: any) => 
            Object.entries(selectedVariant).some(([key, val]) => 
              v.name.toLowerCase() === key.toLowerCase() && v.options.includes(val)
            )
          );
          if (matching) {
      ...
      set((state) => {
        const existingIdx = state.cart.findIndex((item) => item.cartItemId === cartItemId);
        let updatedCart;
        
        if (existingIdx !== -1) {
          updatedCart = [...state.cart];
          const newQty = updatedCart[existingIdx].quantity + quantity;
          updatedCart[existingIdx].quantity = product.stockCount > 0 ? Math.min(newQty, product.stockCount) : newQty;
        } else {
          updatedCart = [...state.cart, { cartItemId, product, quantity, selectedVariant }];
        }
  ...
  updateQuantity: async (cartItemId, quantity) => {
    ...
    if (isLoggedIn) {
      try {
        await apiClient.put(`/api/v1/cart/items/${cartItemId}?quantity=${quantity}`);
        await get().fetchCart();
      } catch (e: any) {
      ...
      set((state) => {
        const updatedCart = state.cart.map((item) => {
          if (item.cartItemId === cartItemId) {
            const capQty = item.product.stockCount > 0 ? Math.min(quantity, item.product.stockCount) : quantity;
  ...
  applyCoupon: async (code) => {
    ...
    if (isLoggedIn) {
      try {
        ...
        const coupon: Coupon = {
          code: response.code,
          discountType: response.type.toLowerCase() === 'percentage' ? 'percentage' : 'flat',
          value: Number(response.discount),
          minSpend: 0,
          description: `Backend Coupon: ${response.code}`
        };
        set({ coupon });
        return { success: true, message: `Coupon ${response.code} applied successfully!` };
      } catch (e: any) {
...
// Wishlist Store Types and Store
...
export const useWishlistStore = create<WishlistStore>((set, get) => ({
  wishlist: typeof window !== 'undefined' ? JSON.parse(localStorage.getItem('sc_wishlist_items') || '[]') : [],
  ...
  fetchWishlist: async () => {
    ...
    try {
      ...
      const products = dbWishlist.map((item: any) => mapBackendProduct(item.product));
