...
export const CartDrawer: React.FC = () => {
  ...
  const { isCartOpen, cart, setCartOpen, updateQuantity, removeFromCart, getTotals } = useCartStore();
  ...
  useEffect(() => {
    const handleOutsideClick = (e: MouseEvent) => {
      if (isCartOpen && containerRef.current && !containerRef.current.contains(e.target as Node)) {
        setCartOpen(false);
      }
    };
            ...
            <div className="flex-grow overflow-y-auto p-4 space-y-4">
              {cart.length === 0 ? (
                <div className="h-full flex flex-col items-center justify-center text-center p-8 space-y-4">
                  ...
                  <div>
                    <h4 className="font-heading font-bold text-base text-swift-dark">Your cart is empty</h4>
                    <p className="text-xs text-swift-mid mt-1">Add items to get started on your shopping journey.</p>
                  </div>
                  <button
                    onClick={() => setCartOpen(false)}
              ...
              <div className="p-4 border-t border-gray-100 bg-gray-50/50 space-y-4">
                <div className="space-y-1.5 text-sm">
                  ...
                  <div className="flex justify-between text-base font-extrabold text-swift-dark">
                    ...
                    <span className="font-mono text-swift-orange">₹{totals.total.toLocaleString('en-IN')}</span>
                  </div>
                </div>
