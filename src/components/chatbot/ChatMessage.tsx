...
import { Bot, User, Calendar, Box, ArrowRight, ExternalLink } from 'lucide-react';
...
export const ChatMessage: React.FC<ChatMessageProps> = ({ message }) => {
  ...
  return (
    <div className={`flex w-full mb-4 px-4 items-start gap-2.5 ${isBot ? 'justify-start' : 'justify-end'}`}>
      {/* Bot Avatar */}
      {isBot && (
      ...
      <div className={`flex flex-col max-w-[75%] gap-1 ${!isBot ? 'items-end' : 'items-start'}`}>
        {/* Simple Text Bubble */}
        {message.text && (
          <div
        ...
        {isBot && message.type === 'order_card' && message.order && (
          <div className="w-full mt-2 bg-white rounded-2xl border border-gray-100 shadow-card p-3 flex flex-col gap-2">
            ...
            <div className="flex items-center justify-between border-t border-gray-50 pt-2 mt-1">
              <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full border ${getStatusBadgeColor(message.order.status)}`}>
            ...
            <button
              onClick={() => navigate(`/orders/track/${message.order?.orderId}`)}
