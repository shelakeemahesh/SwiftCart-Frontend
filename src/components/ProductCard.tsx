...
import { useWishlistStore, useCartStore, useToastStore } from '../store/useSwiftStore';
    ...
    return (
      ...
      >
        ...
        <div className="flex-grow flex flex-col justify-between py-1">
          <div>
            <div className="text-xs font-bold text-swift-mid uppercase tracking-wide">{product.brand}</div>
            ...
            <div className="flex items-center gap-1.5 mt-2">
              <div className="flex items-center bg-swift-orange/10 px-1.5 py-0.5 rounded-button">
                ...
                <span className="text-xs font-bold text-swift-orange ml-1">{product.rating}</span>
              </div>
              <span className="text-xs text-swift-mid">({product.reviewCount} reviews)</span>
            </div>

            {/* Highlights */}
            <ul className="mt-3 space-y-1 hidden md:block">
              {product.highlights.slice(0, 3).map((hl, i) => (
                <li key={i} className="text-xs text-swift-mid flex items-center gap-1.5">
          ...
          <div className="flex flex-wrap items-end justify-between gap-4 mt-4">
            ...
            <div>
              <div className="flex items-baseline gap-2">
                <span className="text-xl font-extrabold text-swift-dark">₹{product.price.toLocaleString('en-IN')}</span>
                {product.mrp > product.price && (
                  <>
                    <span className="text-sm text-swift-mid line-through font-mono">₹{product.mrp.toLocaleString('en-IN')}</span>
                    <span className="text-sm font-bold text-swift-orange">({product.discount}% OFF)</span>
                  </>
                )}
              </div>
              {product.price >= 499 && (
  ...
  return (
    ...
    >
      ...
      <div className="p-3 flex-grow flex flex-col justify-between min-h-[140px]">
        <div>
          <div className="text-[10px] font-bold text-swift-mid uppercase tracking-wider">{product.brand}</div>
          ...
          <div className="flex items-center gap-1 mt-1.5">
            <div className="flex items-center bg-swift-orange/10 px-1 py-0.5 rounded-button">
              ...
              <span className="text-[10px] font-bold text-swift-orange ml-0.5">{product.rating}</span>
            </div>
            <span className="text-[10px] text-swift-mid">({product.reviewCount})</span>
          </div>
        </div>
        ...
        <div className="mt-3 space-y-2">
          <div>
            <div className="flex items-baseline gap-1.5 flex-wrap">
              <span className="text-base font-extrabold text-swift-dark">₹{product.price.toLocaleString('en-IN')}</span>
              {product.mrp > product.price && (
                <>
                  <span className="text-xs text-swift-mid line-through font-mono">₹{product.mrp.toLocaleString('en-IN')}</span>
                  <span className="text-xs font-bold text-swift-orange">-{product.discount}%</span>
                </>
              )}
            </div>
            {product.price >= 499 && (
